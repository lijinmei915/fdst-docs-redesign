...
完整、闭合的静态图片/渐变背景及纯 text-decoration 关键字为 exempt。已识别的组件图片/文字装饰变量链，在没有 FDS/旧色板引用且 fallback 为结构值或单一 url 时保留协议。未知变量、包含 FDS 引用的复合值、动画简写、无法安全解析的布局表达式仍按原规则报告，不把静态豁免扩展到动态内容。exempt 不是通用 CSS 语法校验或视觉验收。

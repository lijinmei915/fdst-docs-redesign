...
约定保留的硬编码使用 `exempt` 进入 JSON 事实和统计，但不进入 HTML 的迁移问题、逐项提示词或人工决策集合。最近档自动替换必须在原因列显示原值到目标值的变化，并在 Token 列和下拉选项中显示 Catalog 注释；精确命中的自动替换直接归入 `auto-replace` / `replaced`，只展示确定 Token，不提供下拉。报告提示不得通过向业务源码插入注释实现。
...
- 仅当 `auto-replace` / `replaced` 的 `selectedToken.match` 为 `nearest` 时显示 Token 选择框；选项必须逐字来自该 finding 的 `candidates`，不得接受候选外 Token。选择具体 Token 即纳入汇总，清空选择即移出汇总。
- 精确命中的自动替换只展示确定性结果，不显示选择框或处理说明；其他需人工处理项显示处理说明输入框，非空说明即纳入汇总，清空说明即移出汇总。

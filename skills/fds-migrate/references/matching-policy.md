...
3. 非颜色值必须能够按对应 Token 类型完整解析；圆角和透明度可按下述专项规则选择最近档，字号、相对行高及其他类型要求候选 `resolvedValue` 与原值精确等价。
...
- Font Size：小于 `12px` 与超过 `48px` 的值保留硬编码并归为 `exempt`；`12px–48px` 仅在属性、层级兼容且解析值精确相等时自动替换；非精确值保持原样，相近候选仅供报告。此边界不受旧快照 nearestAutoReplace 标记影响。
...
最近档自动替换必须在 finding 中记录原值和目标解析值。Markdown 展示值变化、候选及 Catalog 注释，不提供交互控件。源码只改声明值并保留 fallback，不额外插入迁移注释。

...
# 设计参考中的 10 套固定色板；索引 1 采用最终确认参数，两项 8-bit 量化差异在测试末尾显式锁定。
DESIGN_REFERENCE = {
...
class GenerateColorPalettesTest(unittest.TestCase):
    def test_step_1_uses_design_parameters(self) -> None:
        self.assertEqual(tuple(range(12)), PALETTES.STEPS)
    ...
    def test_design_reference_matches_within_hex_quantization(self) -> None:
        ...
        self.assertEqual(
            [
                ("amber", 10, "#763400", "#773400"),
                ("green", 10, "#005A2F", "#005A30"),
            ],
            exact_mismatches,
        )

    def test_step_8_preserves_seed_exactly(self) -> None:
        for family, seed in PALETTES.load_color_seeds().items():
            self.assertEqual(seed, PALETTES.generate_palette(seed)[8], family)
    ...
    def test_dark_map_matches_design_contract(self) -> None:
        ...
        for family, palette in palettes.items():
            self.assertEqual(seeds[family], palette[8], family)
        self.assertEqual("#FFC14D", palettes["yellow"][9])
